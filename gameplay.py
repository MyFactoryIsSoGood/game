...
INIT_PREY = 1  # Травоядные
INIT_SYNTH = 0  # Синтезаторы
INIT_FOOD = 1200  # Органика

AGE_LIMIT = 120  # Продолжительность жизни клеток

FOOD_COST = 20  # Количество энергии за одну клетку органики
SYNTHETIC_COST = 40  # Количество энергии за одну клетку синтетики
...
SYNTH_FOOD_PER_MOVE = 2  # Количество еды, генерируемое синтезтором за один подход
SYNTHESIS_PROBABILITY = 30  # Вероятность того, что на этом ходу синтезатор создаст еду (в процентах)
...
class Prey(Animal):
    ...
    def pass_epoch(self, session):
        self.age += 1
        deltas = self.calculate_move(session)
        if deltas != [0, 0]:
    ...
    def calculate_move(self, session):
        deltas = [0, 0]
        selected = []
        for row in range(self.y_pos - PREY_FOV, self.y_pos + PREY_FOV + 1):
            for cell in range(self.x_pos - PREY_FOV, self.x_pos + PREY_FOV + 1):
                if (HEIGHT_PX // CELL_SIZE) > row >= 0 and (WIDTH_PX // CELL_SIZE) > cell >= 0 and (cell, row) != (
                        self.x_pos, self.y_pos):
                    if isinstance(session.field.canvas[row][cell], Food):
                        sel_x = cell
                        sel_y = row
        ...
        if selected:
            ...
            sel_x, sel_y = session.field.center[0]+randint(-50,50), session.field.center[1]+randint(-50,50)
        ...
        cell = session.field.canvas[self.y_pos + deltas[1]][self.x_pos + deltas[0]]
        if isinstance(cell, Prey) or isinstance(cell, Synth):
            deltas = [0, 0]
        return deltas
...
class Food:
    def __init__(self, x_pos: int, y_pos: int, synthetic=False):
        self.x_pos = x_pos
        self.y_pos = y_pos
...
class Field:
    def __init__(self, width: int, height: int):
        ...
        self.empty = []
        self.center = [0, 0]
        self.food_center = [0, 0]
        self.__update_empty()

    def update_center(self, foods):
        for fd in foods:
            self.center[0] += fd.x_pos
            self.center[1] += fd.y_pos
        self.center[0] = int(self.center[0] / len(foods)) + randint(-20, 20)
        self.center[1] = int(self.center[1] / len(foods)) + randint(-20, 20)
...
class Session:
    ...
    def log(self, fps: int, full=False):
    ...
    def main_loop(self):
        ...
        self.update_arrs()
        self.field.update_center(self.foods)
